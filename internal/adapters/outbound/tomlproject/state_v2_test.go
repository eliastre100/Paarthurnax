...
func TestV2SaveAndLoad(t *testing.T) {
	...
	for _, key := range []string{"version = 2", "[locales]", "[[files]]", "[files.segments]"} {
...
func TestV2SaveAndLoadEmptySnapshot(t *testing.T) {
	path := filepath.Join(t.TempDir(), ".paarthurnax")
	repository := NewRepository(path)
...
func TestV2SaveRejectsInvalidState(t *testing.T) {
